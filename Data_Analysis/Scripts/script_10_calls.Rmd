...
xdata3 <- data.frame(seqID = unique(xdata$Seq1), group=as.character(xdata$Group[which(xdata$Nb_call==1)]),
                     ...
                     dis = xdata$Distance[which(xdata$Nb_call==1)], seqcall = substr(SeqCalls, start = 1, stop = 10),
       ...
       {if(distance1 < 0 || identical(distance1, numeric(0))){
         next
          ...
          {if(distance1 >= peak){
            ...
            {if (!(gram %in% names(xdata3))) {
              next
            }
            else{
                xdata3[xdata3$seqID == as.character(id),gram] <- sum(xdata3[xdata3$seqID == as.character(id),gram], 1)
            }
            }
          }
           else{
...
id_bigram_columns <- grep("bigram_", names(xdata3)) 
id_trigram_columns <- grep("trigram_", names(xdata3))
id_tetragram_columns <- grep("tetragram_", names(xdata3))
id_pentagram_columns <- grep("pentagram_", names(xdata3))

#Create the columns in the dataset
xdata3$slope_bigram <- NA
xdata3$slope_trigram <- NA
xdata3$slope_tetragram <- NA
xdata3$slope_pentagram <- NA
...
for(row in 1:nrow(xdata3)) {
  ...
  total_bigrams <- sum(xdata3[row, id_bigram_columns])
  total_trigrams <- sum(xdata3[row, id_bigram_columns])
  total_tetragrams <- sum(xdata3[row, id_bigram_columns])
  total_pentagrams <- sum(xdata3[row, id_bigram_columns])
  ...
  for (id in id_bigram_columns){
    bi_temp[nrow(bi_temp) + 1,] = c(names(xdata3[id]), xdata3[row,id])
    rownames(bi_temp) <- as.character(bi_temp$gram)
    
    xdata3[row, id] <- bayesian_mean(xdata3[row, id],alpha_bi, total_bigrams, length(id_bigram_columns))
  }
  for (id in id_trigram_columns){
    tri_temp[nrow(tri_temp) + 1,] = c(names(xdata3[id]), xdata3[row,id])
    rownames(tri_temp) <- as.character(tri_temp$gram)
    
    xdata3[row, id] <- bayesian_mean(xdata3[row, id],alpha_tri, total_trigrams, length(id_trigram_columns))
  }
  for (id in id_tetragram_columns){
    tetra_temp[nrow(tetra_temp) + 1,] = c(names(xdata3[id]), xdata3[row,id])
    rownames(tetra_temp) <- as.character(tetra_temp$gram)

    xdata3[row, id] <- bayesian_mean(xdata3[row, id],alpha_tetra, total_tetragrams, length(id_tetragram_columns))
  }
  for (id in id_pentagram_columns){
    penta_temp[nrow(penta_temp) + 1,] = c(names(xdata3[id]), xdata3[row,id])
    rownames(penta_temp) <- as.character(penta_temp$gram)
  
    xdata3[row, id] <- bayesian_mean(xdata3[row, id],alpha_penta, total_pentagrams, length(id_pentagram_columns))
  ...
  xdata3$slope_bigram[row] <- lm(bi_temp ~ c(1:length(bi_temp)))$coefficients[2]
  xdata3$slope_trigram[row] <- lm(tri_temp ~ c(1:length(tri_temp)))$coefficients[2]
  xdata3$slope_tetragram[row] <- lm(tetra_temp ~ c(1:length(tetra_temp)))$coefficients[2]
  xdata3$slope_pentagram[row] <- lm(penta_temp ~ c(1:length(penta_temp)))$coefficients[2]
...
targets <- c("start-A", "start-B", "A-A", "A-B", "A-D", "A-E", "B-A", "B-B", "B-D", "B-E", "D-A", "D-B", "D-D", "D-E", "E-A", "E-B", "E-D", "E-E")
# Integrate a column for each transition probability in the big dataset
xdata3 <- cbind(xdata3,  NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA)
...
targets <- c("tpSA", "tpSB", "tpAA", "tpAB", "tpAD", "tpAE", "tpBA", "tpBB", "tpBD", "tpBE", "tpDA", "tpDB", "tpDD", "tpDE", "tpEA", "tpEB", "tpED", "tpEE")
colnames(xdata3)[(ncol(xdata3)-21) : ncol(xdata3)] <- targets
rm(targets)
